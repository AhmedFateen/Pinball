...
ScoreMultiplier::ScoreMultiplier(Vector2D center, float radius, float multiplier) : center(center), radius(radius), multiplier(multiplier), collidedLastFrame(false) {
}
...
void ScoreMultiplier::updateScore(Ball& ball, Score& score) {
	...
	if (collidedLastFrame)
	{
		score.setScore(static_cast<int>(score.getScore() * multiplier));
	}
}
